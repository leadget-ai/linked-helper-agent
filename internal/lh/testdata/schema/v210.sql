...
CREATE TABLE action_configs(
    ...
    maxActionResultsPerIteration INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS "person_member_distance"(
                id INTEGER PRIMARY KEY,
                person_id INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS "person_mutual_total"(
                id INTEGER PRIMARY KEY,
                person_id INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS "person_network_info"(
                id INTEGER PRIMARY KEY,
                person_id INTEGER NOT NULL,
                following INTEGER, followable INTEGER,
                li_account_id INTEGER NOT NULL,
                created_at DATETIME DEFAULT (STRFTIME('%Y-%m-%dT%H:%M:%fZ', 'NOW')),
                updated_at DATETIME DEFAULT (STRFTIME('%Y-%m-%dT%H:%M:%fZ', 'NOW')),
...
CREATE TABLE IF NOT EXISTS "person_original_mutual"(
                id INTEGER PRIMARY KEY,
                person_id INTEGER NOT NULL,
                first_mutual_full_name TEXT, second_mutual_full_name TEXT,
                li_account_id INTEGER NOT NULL,
                created_at DATETIME DEFAULT (STRFTIME('%Y-%m-%dT%H:%M:%fZ', 'NOW')),
                updated_at DATETIME DEFAULT (STRFTIME('%Y-%m-%dT%H:%M:%fZ', 'NOW')),
                sent_at_to_pas DATETIME,
...
CREATE TABLE IF NOT EXISTS "person_custom_mutual"(
                id INTEGER PRIMARY KEY,
                person_id INTEGER NOT NULL,
                first_mutual_full_name TEXT, second_mutual_full_name TEXT,
                li_account_id INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS "person_mutual"(
                id INTEGER PRIMARY KEY,
                person_id INTEGER NOT NULL,
                first_mutual_full_name TEXT, second_mutual_full_name TEXT,
                li_account_id INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS "action_target_people"(
                id INTEGER PRIMARY KEY,
action_id INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS "action_target_organizations"(
                id INTEGER PRIMARY KEY,
action_id INTEGER NOT NULL,
action_version_id INTEGER NOT NULL,
organization_id INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS "organizations_action_results"(
                id INTEGER PRIMARY KEY,
action_version_id INTEGER NOT NULL,
action_iteration_id INTEGER,
organization_id INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS "chats"(
                id INTEGER PRIMARY KEY, original_id INTEGER, type TEXT NOT NULL, platform TEXT NOT NULL, li_account_id INTEGER NOT NULL, created_at DATETIME DEFAULT (STRFTIME('%Y-%m-%dT%H:%M:%fZ', 'NOW')), updated_at DATETIME DEFAULT (STRFTIME('%Y-%m-%dT%H:%M:%fZ', 'NOW')),
                
                FOREIGN KEY(original_id) REFERENCES chats(id), FOREIGN KEY(li_account_id) REFERENCES li_accounts(id)
            );
CREATE TABLE IF NOT EXISTS "tasks"(
                id INTEGER PRIMARY KEY, type TEXT NOT NULL, status TEXT DEFAULT ('unscheduled'), start_at DATETIME, li_account_id INTEGER NOT NULL, created_at DATETIME DEFAULT (STRFTIME('%Y-%m-%dT%H:%M:%fZ', 'NOW')), updated_at DATETIME DEFAULT (STRFTIME('%Y-%m-%dT%H:%M:%fZ', 'NOW')),
                
                FOREIGN KEY(li_account_id) REFERENCES li_accounts(id)
            );
CREATE TABLE IF NOT EXISTS "time_intervals"(
                id INTEGER PRIMARY KEY, name TEXT NOT NULL, start DATETIME, "end" DATETIME, li_account_id INTEGER NOT NULL,
                
                FOREIGN KEY(li_account_id) REFERENCES li_accounts(id)
            );
CREATE TABLE IF NOT EXISTS "chat_external_ids"(
                id INTEGER PRIMARY KEY, chat_id INTEGER NOT NULL, external_id TEXT NOT NULL, li_account_id INTEGER NOT NULL, created_at DATETIME DEFAULT (STRFTIME('%Y-%m-%dT%H:%M:%fZ', 'NOW')), updated_at DATETIME DEFAULT (STRFTIME('%Y-%m-%dT%H:%M:%fZ', 'NOW')),
                UNIQUE(li_account_id, external_id),
                FOREIGN KEY(chat_id) REFERENCES chats(id), FOREIGN KEY(li_account_id) REFERENCES li_accounts(id)
            );
CREATE TABLE IF NOT EXISTS "message_external_ids"(
                id INTEGER PRIMARY KEY, message_id INTEGER NOT NULL, external_id TEXT NOT NULL, li_account_id INTEGER NOT NULL, created_at DATETIME DEFAULT (STRFTIME('%Y-%m-%dT%H:%M:%fZ', 'NOW')), updated_at DATETIME DEFAULT (STRFTIME('%Y-%m-%dT%H:%M:%fZ', 'NOW')),
                UNIQUE(li_account_id, external_id),
                FOREIGN KEY(message_id) REFERENCES messages(id), FOREIGN KEY(li_account_id) REFERENCES li_accounts(id)
            );
CREATE TABLE IF NOT EXISTS "working_intervals"(
            id INTEGER PRIMARY KEY, campaign_id INTEGER, action_id INTEGER, working_week_day INTEGER NOT NULL, day_and_night INTEGER NOT NULL DEFAULT 0, started_at INTEGER, ended_at INTEGER, li_account_id INTEGER NOT NULL,
            UNIQUE(li_account_id, campaign_id, action_id, working_week_day, started_at, ended_at),
            FOREIGN KEY(campaign_id) REFERENCES campaigns(id), FOREIGN KEY(action_id) REFERENCES actions(id), FOREIGN KEY(li_account_id) REFERENCES li_accounts(id)
        );
CREATE TABLE IF NOT EXISTS "daily_limits"(
            id INTEGER PRIMARY KEY, li_account_id INTEGER NOT NULL, max_limit INTEGER NOT NULL DEFAULT 150,
            UNIQUE(li_account_id),
            FOREIGN KEY(li_account_id) REFERENCES li_accounts(id)
        );
CREATE TABLE IF NOT EXISTS "limit_type_period_max_credits"(
            id INTEGER PRIMARY KEY, limit_type_id INTEGER NOT NULL, period INTEGER NOT NULL, max_credits INTEGER NOT NULL CHECK(max_credits >= 0), is_deleted INTEGER NOT NULL DEFAULT 0, read_only INTEGER NOT NULL DEFAULT 0, license_feature_set TEXT DEFAULT 'any' CHECK(license_feature_set in ('standard', 'pro', 'any')), li_account_id INTEGER NOT NULL, created_at DATETIME DEFAULT (STRFTIME('%Y-%m-%dT%H:%M:%fZ', 'NOW')), updated_at DATETIME DEFAULT (STRFTIME('%Y-%m-%dT%H:%M:%fZ', 'NOW')),
            UNIQUE(li_account_id, limit_type_id, period, license_feature_set),
            FOREIGN KEY(limit_type_id) REFERENCES limit_types(id), FOREIGN KEY(li_account_id) REFERENCES li_accounts(id)
        );
CREATE TABLE IF NOT EXISTS "limit_type_credits_used"(
            id INTEGER PRIMARY KEY, limit_type_id INTEGER NOT NULL, used_credits_count INTEGER NOT NULL, result_id INTEGER, action_version_id INTEGER, li_account_id INTEGER NOT NULL, created_at DATETIME DEFAULT (STRFTIME('%Y-%m-%dT%H:%M:%fZ', 'NOW')), updated_at DATETIME DEFAULT (STRFTIME('%Y-%m-%dT%H:%M:%fZ', 'NOW')),
...
CREATE TABLE IF NOT EXISTS "method_call_cache"(
            id INTEGER PRIMARY KEY, li_account_id INTEGER NOT NULL, method TEXT NOT NULL, args TEXT NOT NULL, result TEXT, created_at DATETIME DEFAULT (STRFTIME('%Y-%m-%dT%H:%M:%fZ', 'NOW')), expires_at DATETIME,
...
CREATE TABLE IF NOT EXISTS "person_original_mini_profile"(
            id INTEGER PRIMARY KEY,
            person_id INTEGER NOT NULL,
            first_name TEXT NOT NULL,
            last_name TEXT,
...
CREATE TABLE IF NOT EXISTS "person_in_campaigns_history"(
    id INTEGER PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS "organization_in_campaigns_history"(
    id INTEGER PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS "person_connect"(
    id INTEGER PRIMARY KEY,
    person_id INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS "chat_messages_cursor"(
            id INTEGER PRIMARY KEY,
            chat_id INTEGER NOT NULL,
            message_id INTEGER NOT NULL,
...
CREATE TABLE li_account_flags(
        ...
        updated_at DATETIME DEFAULT (STRFTIME('%Y-%m-%dT%H:%M:%fZ', 'NOW')),
        FOREIGN KEY (li_account_id) REFERENCES li_accounts(id),
        UNIQUE (li_account_id, flag)
    );
CREATE TABLE deferred_deduplications(
    id INTEGER PRIMARY KEY,
    li_account_id INTEGER NOT NULL,
    ...
    FOREIGN KEY (li_account_id) REFERENCES li_accounts(id),
    FOREIGN KEY (deduplication_id) REFERENCES deduplications(id),
    UNIQUE(li_account_id, deduplication_id)
);
CREATE TABLE deferred_deduplications_organizations(
    id INTEGER PRIMARY KEY,
    li_account_id INTEGER NOT NULL,
    deduplication_id INTEGER NOT NULL,
    is_processed INTEGER DEFAULT 0,
...
CREATE TABLE campaign_events_store (
        id TEXT PRIMARY KEY,
        type TEXT NOT NULL,
        aggregate_id INTEGER NOT NULL,
        aggregate_version INTEGER NOT NULL,
...
CREATE INDEX organization_external_id_identifiers_organization_external_id_sent_at_to_pas_idx ON organization_external_id_identifiers(organization_external_id, sent_at_to_pas) WHERE sent_at_to_pas IS NULL;
CREATE INDEX person_external_id_identifiers_person_external_id_sent_at_to_pas_idx ON person_external_id_identifiers(person_external_id, sent_at_to_pas) WHERE sent_at_to_pas IS NULL;
CREATE INDEX person_address_person_id_sent_at_to_pas_idx ON person_address(person_id, sent_at_to_pas) WHERE sent_at_to_pas IS NULL;
CREATE INDEX person_badges_person_id_sent_at_to_pas_idx ON person_badges(person_id, sent_at_to_pas) WHERE sent_at_to_pas IS NULL;
CREATE INDEX person_birthday_person_id_sent_at_to_pas_idx ON person_birthday(person_id, sent_at_to_pas) WHERE sent_at_to_pas IS NULL;
CREATE INDEX person_connections_info_person_id_sent_at_to_pas_idx ON person_connections_info(person_id, sent_at_to_pas) WHERE sent_at_to_pas IS NULL;
CREATE INDEX person_education_person_id_sent_at_to_pas_idx ON person_education(person_id, sent_at_to_pas) WHERE sent_at_to_pas IS NULL;
CREATE INDEX person_email_person_id_sent_at_to_pas_idx ON person_email(person_id, sent_at_to_pas) WHERE sent_at_to_pas IS NULL;
CREATE INDEX person_followers_person_id_sent_at_to_pas_idx ON person_followers(person_id, sent_at_to_pas) WHERE sent_at_to_pas IS NULL;
CREATE INDEX person_industry_person_id_sent_at_to_pas_idx ON person_industry(person_id, sent_at_to_pas) WHERE sent_at_to_pas IS NULL;
CREATE INDEX person_languages_person_id_sent_at_to_pas_idx ON person_languages(person_id, sent_at_to_pas) WHERE sent_at_to_pas IS NULL;
CREATE INDEX person_location_person_id_sent_at_to_pas_idx ON person_location(person_id, sent_at_to_pas) WHERE sent_at_to_pas IS NULL;
CREATE INDEX person_messengers_person_id_sent_at_to_pas_idx ON person_messengers(person_id, sent_at_to_pas) WHERE sent_at_to_pas IS NULL;
CREATE INDEX person_original_current_position_person_id_sent_at_to_pas_idx ON person_original_current_position(person_id, sent_at_to_pas) WHERE sent_at_to_pas IS NULL;
CREATE INDEX person_original_mini_profile_person_id_sent_at_to_pas_idx ON person_original_mini_profile(person_id, sent_at_to_pas) WHERE sent_at_to_pas IS NULL;
CREATE INDEX person_phone_numbers_person_id_sent_at_to_pas_idx ON person_phone_numbers(person_id, sent_at_to_pas) WHERE sent_at_to_pas IS NULL;
CREATE INDEX person_skill_person_id_sent_at_to_pas_idx ON person_skill(person_id, sent_at_to_pas) WHERE sent_at_to_pas IS NULL;
CREATE INDEX person_summary_person_id_sent_at_to_pas_idx ON person_summary(person_id, sent_at_to_pas) WHERE sent_at_to_pas IS NULL;
CREATE INDEX person_third_party_emails_person_id_sent_at_to_pas_idx ON person_third_party_emails(person_id, sent_at_to_pas) WHERE sent_at_to_pas IS NULL;
CREATE INDEX person_twitters_person_id_sent_at_to_pas_idx ON person_twitters(person_id, sent_at_to_pas) WHERE sent_at_to_pas IS NULL;
CREATE INDEX person_websites_person_id_sent_at_to_pas_idx ON person_websites(person_id, sent_at_to_pas) WHERE sent_at_to_pas IS NULL;
CREATE INDEX organization_extra_organization_id_sent_at_to_pas_idx ON organization_extra(organization_id, sent_at_to_pas) WHERE sent_at_to_pas IS NULL;
CREATE INDEX organization_industries_organization_id_sent_at_to_pas_idx ON organization_industries(organization_id, sent_at_to_pas) WHERE sent_at_to_pas IS NULL;
CREATE INDEX organization_mini_profile_organization_id_sent_at_to_pas_idx ON organization_mini_profile(organization_id, sent_at_to_pas) WHERE sent_at_to_pas IS NULL;
CREATE INDEX organization_specialities_organization_id_sent_at_to_pas_idx ON organization_specialities(organization_id, sent_at_to_pas) WHERE sent_at_to_pas IS NULL;
...
CREATE INDEX li_account_flags_flag_value_idx ON li_account_flags(flag, value);
CREATE INDEX deferred_dedup_li_account_id_is_processed_idx ON deferred_deduplications(li_account_id, is_processed);
CREATE INDEX deferred_dedup_orgs_li_account_id_is_processed_idx ON deferred_deduplications_organizations(li_account_id, is_processed);
...
CREATE TRIGGER collections_updated_at_trigger AFTER UPDATE
        ...
        AND (SELECT STRFTIME('%Y-%m-%dT%H:%M:%fZ', 'NOW')) > OLD.updated_at;
        END;
CREATE INDEX action_versions_exclude_list_id_idx
        ON action_versions(exclude_list_id) WHERE exclude_list_id IS NOT NULL;
CREATE INDEX campaign_versions_exclude_list_id_idx
        ON campaign_versions(exclude_list_id) WHERE exclude_list_id IS NOT NULL;
CREATE INDEX actions_campaign_id_idx
        ON actions(campaign_id);
CREATE INDEX pich_camp_li_person_not_del_idx
        ON person_in_campaigns_history (campaign_id, action_target_li_account_id, person_id)
        WHERE action_order_id != -999;
CREATE INDEX pich_act_li_person_not_del_idx
        ON person_in_campaigns_history (action_id, action_target_li_account_id, person_id)
        WHERE action_order_id != -999;
CREATE INDEX pich_act_person_ver_status_idx
        ON person_in_campaigns_history (action_id, person_id, action_target_action_version_id, result_status);
CREATE INDEX oich_camp_li_org_not_del_idx
        ON organization_in_campaigns_history (campaign_id, action_target_li_account_id, organization_id)
        WHERE action_order_id != -999;
CREATE INDEX oich_act_li_org_not_del_idx
        ON organization_in_campaigns_history (action_id, action_target_li_account_id, organization_id)
        WHERE action_order_id != -999;
CREATE INDEX oich_act_org_ver_status_idx
        ON organization_in_campaigns_history (action_id, organization_id, action_target_action_version_id, result_status);
...
CREATE TABLE IF NOT EXISTS "person_custom_fields_versions_logs"(
            id INTEGER PRIMARY KEY,
            person_id INTEGER NOT NULL,
            campaign_id INTEGER,
            campaign_id_uniq_constrain INTEGER NOT NULL DEFAULT 0,
...
CREATE TRIGGER person_custom_fields_insert_trigger
            ...
            BEGIN
                ...
                INSERT INTO person_custom_fields_versions_logs(
                    person_id,campaign_id,campaign_id_uniq_constrain,action_id,action_id_uniq_constrain,level,field_name,field_content,field_type,created_at,updated_at,
                    person_custom_fields_id,
                    version_id,
                    status
                )
                VALUES(
                    NEW.person_id, NEW.campaign_id, NEW.campaign_id_uniq_constrain, NEW.action_id, NEW.action_id_uniq_constrain, NEW.level, NEW.field_name, NEW.field_content, NEW.field_type, NEW.created_at, NEW.updated_at,
                    NEW.id,    
                    (SELECT
                        id
...
CREATE TRIGGER person_custom_fields_delete_trigger
            ...
            BEGIN
                ...
                VALUES(
                    OLD.person_id, OLD.campaign_id, OLD.campaign_id_uniq_constrain, OLD.action_id, OLD.action_id_uniq_constrain, OLD.level, OLD.field_name, OLD.field_content, OLD.field_type, OLD.created_at, OLD.updated_at,
                    OLD.id,    
                    (SELECT
                        id
                    FROM person_custom_fields_versions
            ...
            END;
CREATE INDEX pich_li_account_result_status_target_state_action_idx
        ON person_in_campaigns_history (
        ...
        WHERE action_order_id > -999;
CREATE INDEX oich_li_account_result_status_target_state_action_idx
        ON organization_in_campaigns_history (
            action_target_li_account_id,
            result_status,
            action_add_to_target_state,
            action_id
        ...
        WHERE action_order_id > -999;
CREATE INDEX action_results_version_iter_not_null_idx
        ON action_results(action_version_id, action_iteration_id DESC)
        WHERE action_iteration_id IS NOT NULL;
CREATE INDEX organizations_action_results_version_iter_not_null_idx
        ON organizations_action_results(action_version_id, action_iteration_id DESC)
        WHERE action_iteration_id IS NOT NULL;
